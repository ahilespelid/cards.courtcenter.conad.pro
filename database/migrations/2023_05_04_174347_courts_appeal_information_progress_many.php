...
class CourtsAppealInformationProgressMany extends Migration
{
    ...
    public function up(){
        Schema::create('courts_appeal_information_progress__many', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('courts_appeal_information_progress_id')->unsigned();
            
            $table->rememberToken();
            $table->timestamps();

           $table->foreign('courts_appeal_id')->references('id')->on('courts_appeal')->onDelete('cascade');
           $table->foreign('courts_appeal_information_progress_id')->references('id')->on('courts_appeal_information_progress')->onDelete('cascade'); 
        });
    }
